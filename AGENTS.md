...
**Runtime:** Cloudflare Workers + React Router v7 (SSR)
...
- Environment vars: client-safe in `.env` (and `wrangler.jsonc` vars), secrets in `.env.local`.
- Never commit API keys, R2 credentials, OAuth secrets, or webhook secrets.
