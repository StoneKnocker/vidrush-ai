 ...
 */
export const r2FileExists = async (filePath: string): Promise<boolean> => {
  const response = await getR2Client().fetch(getR2ObjectEndpoint(filePath), {
    method: "HEAD",
  });
  ...
  if (!response.ok) {
    console.error("Error checking R2 file:", {
      filePath: normalizeR2Key(filePath),
      status: response.status,
      statusText: response.statusText,
    });
    return false;
 ...
 */
export const getR2FileMetadata = async (filePath: string) => {
  const response = await getR2Client().fetch(getR2ObjectEndpoint(filePath), {
    method: "HEAD",
  });
  ...
  if (!response.ok) {
    console.error("Error getting R2 file metadata:", {
      filePath: normalizeR2Key(filePath),
      status: response.status,
      statusText: response.statusText,
    });
