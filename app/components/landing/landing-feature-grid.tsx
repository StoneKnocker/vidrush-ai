      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {section.items.map((item) => (
            <article
              key={item.title}
              className="hover:-translate-y-1 rounded-lg border bg-card p-6 transition-all duration-300 hover:border-primary/70 hover:shadow-[0_0_15px_rgba(92,88,85,0.2)]"
            >
              <LandingIconBadge>
              ...
              <div className="mt-5">
                ...
                <div className="mt-3 text-muted-foreground text-sm leading-7">
                  <RichText text={item.description} />
                </div>
              </div>
              {item.bullets?.length ? (
                <ul className="mt-5 flex flex-col gap-2 text-muted-foreground text-sm">
