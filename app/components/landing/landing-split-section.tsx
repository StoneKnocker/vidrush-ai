      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          <div className={cn(imageFirst ? "lg:order-2" : "lg:order-1")}>
            ...
            <div className="flex flex-col gap-4">
              {section.items.map((item) => (
                <article
                  key={item.title}
                  className="rounded-lg border border-[#3d3a39] bg-[#101010]/90 p-5 shadow-[0_0_15px_rgba(92,88,85,0.12)] transition-all duration-300 hover:border-[#00d992]/60"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-medium text-[#f2f2f2] text-lg">
                        {item.title}
                      </h3>
                      <div className="mt-2 text-[#b8b3b0] text-sm leading-7">
                      ...
                      {item.bullets?.length ? (
                        <ul className="mt-4 flex flex-col gap-2">
                          {item.bullets.map((bullet) => (
                            <li
                              key={bullet}
                              className="flex items-start gap-2 text-[#b8b3b0] text-sm"
                            >
                              <LandingIcon
                                name="check"
          ...
          <div className={cn(imageFirst ? "lg:order-1" : "lg:order-2")}>
            <div className="relative overflow-hidden rounded-lg border border-[#3d3a39] bg-[#101010] shadow-[0_20px_60px_rgba(0,0,0,0.7)]">
              {section.image ? (
                <img
              ...
              <div className="pointer-events-none absolute right-5 bottom-5 left-5 h-px bg-[#00d992]/70 shadow-[0_0_8px_rgba(0,217,146,0.4)]" />
            </div>
          </div>
        </div>
      </div>
