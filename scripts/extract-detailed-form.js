...
(() => {
  ...
  function findButtonsByText(text) {
    return [...form.querySelectorAll("button")].filter((b) =>
      b.textContent.trim().includes(text),
    );
  }

  function findElementByText(selector, text) {
    return [...form.querySelectorAll(selector)].find((el) =>
  ...
  // AI model dropdown
  const modelBtn =
    findElementByText("button", "Seedance 2.0") ||
    form.querySelector('[role="combobox"]');
  // Find prompt textarea
  const promptArea = [...form.querySelectorAll("textarea")].find(
    (t) =>
      t.placeholder?.includes("Type @") ||
      t.placeholder?.includes("prompt") ||
      t.className?.includes("prompt"),
  );
  // Resolution buttons
  const resBtns = allButtons.filter((b) =>
    ["480p", "720p", "1080p", "4K"].includes(b.textContent.trim()),
  );
  // Duration buttons
  const durBtns =
    allButtons.filter((b) => ["5s"].includes(b.textContent.trim())) ||
    allButtons.filter((b) => b.textContent.trim().match(/^\d+s$/));
  // Aspect ratio buttons
  const arBtns = allButtons.filter((b) =>
    ["Auto", "16:9", "9:16", "4:3", "3:4", "21:9", "1:1"].includes(
      b.textContent.trim(),
    ),
  );
  // Generate
  ...
  // Advanced
  const advancedBtn = allButtons.find((b) =>
    b.textContent.trim().includes("Advanced"),
  );
  // Return last frame
  const returnLastFrame = allButtons.find((b) =>
    b.textContent.trim().includes("Return Last Frame"),
  );
  // Select virtual portrait
  const virtualPortrait = allButtons.find((b) =>
    b.textContent.trim().includes("Select Virtual Portrait"),
  );
  ...
  labels.forEach((lbl) => {
    const sections = [...form.querySelectorAll("div")].filter(
      (div) =>
        div.textContent.trim().startsWith(lbl) && div.children.length > 0,
    );
    if (sections.length) {
      uploadSections.push({
        label: lbl,
        elements: sections.slice(0, 2).map(describe),
      });
    }
  });
  ...
  const guideSection =
    ...
    [...container.children[1].querySelectorAll("div")].find((div) =>
      div.textContent.includes("Multi Reference Guide"),
    );
  ...
  return JSON.stringify(
    {
      currentTab: tabs
        .find((b) => b.classList.contains("bg-primary"))
        ?.textContent?.trim(),
